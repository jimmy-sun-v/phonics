...
class TestProgressTracking:
    ...
    def test_record_attempt_auto_increments(self, session, phoneme):
        a1 = record_attempt(session.session_id, "zz", 0.5)
        a2 = record_attempt(session.session_id, "zz", 0.7)
        a3 = record_attempt(session.session_id, "zz", 0.9)
    ...
    def test_get_attempts_for_phoneme(self, session, phoneme):
        record_attempt(session.session_id, "zz", 0.5)
        record_attempt(session.session_id, "zz", 0.7)
        attempts = get_attempts_for_phoneme(session.session_id, "zz")
        assert len(attempts) == 2

    def test_get_progress(self, session, phoneme):
        record_attempt(session.session_id, "zz", 0.9)
