...
class TestFeedbackStrategy:
    ...
    def test_first_attempt_high_confidence_encourage(self, session, phoneme):
        ctx = determine_feedback_strategy(session.session_id, "zz", 0.85)
    ...
    def test_first_attempt_low_confidence_guide(self, session, phoneme):
        ctx = determine_feedback_strategy(session.session_id, "zz", 0.3)
        assert ctx.strategy == FeedbackStrategy.GUIDE
        assert len(ctx.hints) > 0

    def test_second_attempt_guide(self, session, phoneme):
        SpeechAttempt.objects.create(session=session, phoneme=phoneme, confidence=0.4, attempt_number=1)
        ctx = determine_feedback_strategy(session.session_id, "zz", 0.5)
        assert ctx.strategy == FeedbackStrategy.GUIDE

    def test_three_plus_attempts_adjust(self, session, phoneme):
        for i in range(1, 4):
            SpeechAttempt.objects.create(session=session, phoneme=phoneme, confidence=0.3, attempt_number=i)
        ctx = determine_feedback_strategy(session.session_id, "zz", 0.4)
    ...
    def test_hints_never_discouraging(self, session, phoneme):
        for i in range(1, 6):
            SpeechAttempt.objects.create(session=session, phoneme=phoneme, confidence=0.2, attempt_number=i)
        ctx = determine_feedback_strategy(session.session_id, "zz", 0.2)
        all_hints = " ".join(ctx.hints).lower()
